 ...
 */
 ...
 contract PublicCertificatesPool is MultiSigCertificatesPool, ManagedBaseContract {
    ...
    override payable
    {
        ITreasury(_managerContract.retrieveTransparentProxies()[uint256(Library.TransparentProxies.Treasury)]).pay{value:msg.value}(Library.Prices.NewProvider);
    ...
    function payBack(bytes32 entityInBytes, bool validatedOrRejected) internal
    {
        ...
        for(uint i=0; i < Voters.length; i++){
            ITreasury(_managerContract.retrieveTransparentProxies()[uint256(Library.TransparentProxies.Treasury)]).getRefund(Voters[i], Voters.length);
        }
    }

    function onBeforeAddCertificate() internal override 
    {
        ITreasury(_managerContract.retrieveTransparentProxies()[uint256(Library.TransparentProxies.Treasury)]).pay{value:msg.value}(Library.Prices.NewCertificate);
    }

    function retrieveAddCertificatePriceWei() external override view returns(uint256)
    {
        (, , , uint Price, ) = ITreasury(_managerContract.retrieveTransparentProxies()[uint256(Library.TransparentProxies.Treasury)]).retrieveSettings();
        return IPriceConverter(_managerContract.retrieveTransparentProxies()[uint256(Library.TransparentProxies.PriceConverter)]).fromUSDToETH(Price);
    }
