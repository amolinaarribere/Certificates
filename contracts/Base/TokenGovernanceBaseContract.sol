...
abstract contract TokenGovernanceBaseContract is ITokenEventSubscriber, SignatureBaseContract, ManagedBaseContract {
    ...
    modifier isAuthorizedToPropose(address addr){
        ...
        else 
        {
            ...
            (, , uint8 MinWeightPropPerc) = IPropositionSettings(_managerContract.retrievePropositionSettingsProxy()).retrieveSettings();
    ...
    {
        (uint256 PropLifeTime, uint8 PropThresPer, ) = IPropositionSettings(_managerContract.retrievePropositionSettingsProxy()).retrieveSettings();
